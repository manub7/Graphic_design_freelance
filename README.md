...
This project also ships with a `render.yaml` blueprint for deploying to [Render](https://render.com) with a [Supabase](https://supabase.com) Postgres database.

1. **Database**: create a Supabase project, then copy the connection string from Project Settings > Database (use the "Connection pooling" / transaction-mode URI for a web service). It will be used as `DATABASE_URL`.
2. **Render service**: push this repo to GitHub, then in Render choose "New > Blueprint" and point it at the repo — it will read `render.yaml` and provision the web service automatically.
3. **Environment variables**: Render will prompt for the variables marked `sync: false` in `render.yaml`. At minimum set:
   - `DATABASE_URL` — the Supabase connection string
   - `STRIPE_PUBLIC_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WH_SECRET` — from your Stripe dashboard
   - `EMAIL_HOST_USER`, `EMAIL_HOST_PASS` — a Gmail address + app password (or leave unset and adjust `EMAIL_BACKEND`)
   - Optionally `USE_AWS`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_STORAGE_BUCKET_NAME`, `AWS_S3_REGION_NAME` for media storage. These also work with Supabase Storage's S3-compatible endpoint — additionally set `AWS_S3_ENDPOINT_URL` and `AWS_S3_CUSTOM_DOMAIN` to your Supabase Storage bucket's S3 endpoint/domain.
   `SECRET_KEY` is auto-generated by the blueprint. Do **not** set `DEVELOPMENT` in production — its absence is what puts the app into production mode (`DEBUG=False`).
4. **Static/media files**: static files are served via [WhiteNoise](https://whitenoise.readthedocs.io/) (no extra config needed). Media uploads need either the AWS/Supabase Storage variables above, or they will not persist across deploys since Render's filesystem is ephemeral.
5. **First deploy**: the blueprint's `preDeployCommand` runs migrations automatically. Afterwards, create a superuser with `render shell` (or the Render dashboard's shell) via `python manage.py createsuperuser`.
6. **Site domain**: once you know your Render URL, update the Sites framework entry (`/admin/sites/site/1/`) to that domain — django-allauth uses it to build links in confirmation/reset emails.
